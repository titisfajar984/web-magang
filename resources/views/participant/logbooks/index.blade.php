        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-gray-800">
              {{ $logbook->application->internship->title ?? '-' }}
            </td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-center">
              <div class="flex justify-center items-center space-x-3">
                <a href="{{ route('participant.logbooks.edit', $logbook) }}"
                   class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-yellow-600 bg-yellow-50
                          hover:bg-yellow-100 rounded-md transition h-[36px]"
                   title="Edit">
                  <i data-feather="edit" class="w-4 h-4 mr-1.5"></i> Edit
                </a>
                ...
                <form method="POST" action="{{ route('participant.logbooks.destroy', $logbook) }}"
                  ...
                  @method('DELETE')
                  <button type="submit"
                          class="inline-flex items-center px-3 py-1.5 text-sm font-medium text-red-600 bg-red-50
                                 hover:bg-red-100 rounded-md transition h-[36px]"
                          title="Hapus">
                    <i data-feather="trash-2" class="w-4 h-4 mr-1.5"></i> Hapus
                  </button>
                </form>
              </div>
            </td>
